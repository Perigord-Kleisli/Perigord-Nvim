...
local a, tsref_rename, Func, which_key, telescope_builtin, telescope_themes, tsref_navigation, legendary = autoload("aniseed.core"), autoload("nvim-treesitter-refactor.smart_rename"), autoload("Functions"), autoload("which-key"), autoload("telescope.builtin"), autoload("telescope.themes"), autoload("nvim-treesitter-refactor.navigation"), autoload("legendary")
...
local function has_lsp()
  for _, key in ipairs(vim.lsp.buf_get_clients()) do
    if not ((key.name == "null-ls") or (key.name == "copilot")) then
      return true
    else
    end
  end
